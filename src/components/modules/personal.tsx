...
export function ClockModule() {
  ...
  const time = new Intl.DateTimeFormat('fr-FR', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: timeFormat === '12h', timeZone: timezone }).format(now)
  return <Widget id="clock" title="Maintenant" icon={<TimerReset size={18} />}>
    ...
    {mode === 'digital' ? <div className="clock-digital"><strong>{time}</strong><span>{new Intl.DateTimeFormat('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', timeZone: timezone }).format(now)}</span><small>{timezone.replace('_', ' ')}</small></div> : <div className="clock-analog-wrap"><AnalogClock date={now} /><span>{time}</span></div>}
  </Widget>
}
...
export function CalendarModule() {
  ...
  const EventList=({dates}:{dates:string[]})=><div className={`calendar-agenda ${view}`}>{dates.map(date=>{const dayEvents=events.filter(e=>e.date===date);return <div key={date} className="agenda-day"><header><b>{formatDate(date,{weekday:'short',day:'numeric',month:view==='day'?'long':'short'})}</b><span>{dayEvents.length} événement{dayEvents.length!==1?'s':''}</span></header>{dayEvents.length?dayEvents.map(event=><div className="agenda-event" key={event.id} style={{borderLeftColor:event.color}}><button onClick={()=>edit(event.id,event.title)}><b>{event.title}</b><small>{event.time}</small></button><IconButton label="Modifier" onClick={()=>edit(event.id,event.title)}><Pencil size={12}/></IconButton><IconButton label="Supprimer" onClick={()=>removeEvent(event.id)}><Trash2 size={12}/></IconButton></div>):<small className="agenda-empty">Libre</small>}</div>})}</div>
