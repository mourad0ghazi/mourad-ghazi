...
import { daysUntil, downloadFile, formatDate } from '../../utils/helpers'
...
export function NotesModule({ extended = false }: { extended?: boolean }) {
  ...
  return <Widget id="notes" title="Notes rapides" icon={<NotebookPen size={18} />} action={<IconButton label="Ajouter une note" onClick={addNote}><Plus size={17} /></IconButton>}>
    <div className={`notes-list ${extended ? 'notes-grid' : ''}`}>{notes.slice(0, extended ? 20 : 3).map((note) => <article className="note-card" key={note.id}>
      ...
      <footer><span>{note.pinned ? 'Épinglée · ' : ''}{formatDate(note.updatedAt, { day: 'numeric', month: 'short' })}</span><IconButton label="Supprimer la note" onClick={() => removeNote(note.id)}><Trash2 size={13} /></IconButton></footer>
    </article>)}</div>
  </Widget>
}
...
export function HabitsModule({ extended = false }: { extended?: boolean }) {
  ...
  const toggle = usePersonalStore((s) => s.toggleHabit), add=usePersonalStore(s=>s.addHabit), remove=usePersonalStore(s=>s.removeHabit)
  return <Widget id="habits" title="Habitudes" icon={<Flame size={18} />} action={extended?<IconButton label="Nouvelle habitude" onClick={()=>{const name=prompt('Nom de la nouvelle habitude')?.trim();if(name)add(name)}}><Plus size={16}/></IconButton>:undefined}>
    <div className="habit-days"><span /><span />{week.map((date) => <small key={date}>{new Intl.DateTimeFormat('fr-FR', { weekday: 'narrow' }).format(new Date(date + 'T12:00:00'))}</small>)}</div>
...
export function JournalModule({ extended = false }: { extended?: boolean }) {
  ...
  return <Widget id="journal" title="Journal" icon={<Sparkles size={18} />}>
    ...
    {extended && <div className="journal-history">{entries.map((entry) => <article key={entry.id}><span>{moods[entry.mood - 1]}</span><div><b>{formatDate(entry.date, { weekday: 'long', day: 'numeric', month: 'long' })}</b><p>{entry.content}</p></div><IconButton label="Supprimer" onClick={() => remove(entry.id)}><Trash2 size={14} /></IconButton></article>)}</div>}
  </Widget>
}
