...
import { Bell, CheckCircle2, ChevronDown, FileSpreadsheet, LockKeyhole, Menu, Moon, Plus, Search, Settings, Sun, X } from 'lucide-react'
...
export function Header() {
  ...
  const settings = useSettingsStore(), updateSettings = useSettingsStore((s) => s.update), date = useDateFormatter()
  ...
  const results = useMemo<SearchResult[]>(() => {
    ...
    return [
      ...
      ...transactions.filter((x) => `${x.title} ${x.category}`.toLowerCase().includes(q)).map((x): SearchResult => ({ id: x.id, type: 'Transaction', title: x.title, subtitle: formatCurrency(x.amount, settings.currency, settings.hideAmounts, currencyFormatOptions(settings)), target: 'finances' })),
      ...events.filter((x) => x.title.toLowerCase().includes(q)).map((x): SearchResult => ({ id: x.id, type: 'Événement', title: x.title, subtitle: date(x.date, { day: '2-digit', month: '2-digit', year: 'numeric' }), target: 'personal' })),
  ...
  }, [query, tasks, notes, transactions, events, goals, settings.currency, settings.hideAmounts, settings.amountDecimals, settings.currencyDisplay, settings.language, date])
