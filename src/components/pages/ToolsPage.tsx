...
function exportAll() {
  const payload = { exportedAt: new Date().toISOString(), version: 4, settings: useSettingsStore.getState(), finance: useFinanceStore.getState(), financePlanning: useFinancePlanningStore.getState(), personal: usePersonalStore.getState(), dashboard: useDashboardStore.getState() }
...
function ToolWorkspace({ id }: { id: ToolId }) {
  const transactions = useFinanceStore((s) => s.transactions), settings = useSettingsStore(), update = useSettingsStore((s) => s.update), events = usePersonalStore((s) => s.events), showToast = useUIStore((s) => s.showToast)
...
const importedSettingLabels:Record<string,string>={currency:'Devise',language:'Langue',timezone:'Fuseau horaire',dateFormat:'Format de date',timeFormat:'Format de l’heure',density:'Densité',amountDecimals:'Décimales des prix',currencyDisplay:'Affichage de la devise',theme:'Thème'}
...
function previewColumnName(index:number){let result='';for(let value=index+1;value>0;value=Math.floor((value-1)/26))result=String.fromCharCode((value-1)%26+65)+result;return result}
function SheetPreview({ sheet }: { sheet: ExcelSheet }) {
  const rows=sheet.rows.map((row,index)=>({row,rowNumber:sheet.rowNumbers?.[index]??index+1})).filter(({row})=>row.some((value)=>value!==null&&value!==undefined&&String(value).trim()!=='')).slice(0,7)
  const columns=[...new Set(rows.flatMap(({row})=>Object.keys(row).map(Number).filter((column)=>row[column]!==null&&row[column]!==undefined&&String(row[column]).trim()!=='')))].sort((first,second)=>first-second).slice(0,10)
  return <div className="excel-preview-table" role="region" aria-label={`Aperçu de ${sheet.name}`}><table><thead><tr><th>Ligne</th>{columns.map((column)=><th key={column}>{previewColumnName(column)}</th>)}</tr></thead><tbody>{rows.map(({row,rowNumber})=><tr key={rowNumber}><td className="row-coordinate">{rowNumber}</td>{columns.map((column)=><td key={column}>{String(row[column]??'')}</td>)}</tr>)}</tbody></table></div>
}

function ExcelImportWorkspace() {
  const inputRef=useRef<HTMLInputElement>(null),showToast=useUIStore((s)=>s.showToast),setView=useUIStore((s)=>s.setView),date=useDateFormatter()
  const [plan,setPlan]=useState<ExcelImportPlan|null>(null),[fileName,setFileName]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState(''),[mode,setMode]=useState<ImportMode>('replace'),[adapt,setAdapt]=useState(true),[sheetIndex,setSheetIndex]=useState(0),[applied,setApplied]=useState(false),[archiveInfo,setArchiveInfo]=useState<WorkbookArchiveInfo>()
  const analyze=async(file?:File)=>{if(!file)return;setBusy(true);setError('');setPlan(null);setApplied(false);setFileName(file.name);try{const {parseExcelFile,buildExcelImportPlan}=await import('../../utils/excel');const workbook=await parseExcelFile(file),next=buildExcelImportPlan(workbook);setPlan(next);setSheetIndex(0)}catch(reason){setError(reason instanceof Error?reason.message:'Impossible de lire ce fichier Excel.')}finally{setBusy(false);if(inputRef.current)inputRef.current.value=''}}
  useEffect(()=>{const queuedFile=takeQueuedExcelFile();if(queuedFile)void analyze(queuedFile);void getWorkbookArchiveInfo().then(setArchiveInfo).catch(()=>undefined)},[])
  const reopenArchive=async()=>{setBusy(true);setError('');try{const archived=await loadWorkbookArchive();if(!archived)return;const {buildExcelImportPlan}=await import('../../utils/excel');setFileName(archived.fileName);setPlan(buildExcelImportPlan(archived.workbook));setSheetIndex(0);setApplied(false)}catch(reason){setError(reason instanceof Error?reason.message:'Impossible de relire l’archive locale.')}finally{setBusy(false)}}
  const apply=async()=>{if(!plan||!plan.totalRecords)return;try{const archived=await saveWorkbookArchive(fileName,plan.workbook);setArchiveInfo(archived)}catch{showToast('Archive complète non enregistrée faute d’espace navigateur ; vous pouvez encore la télécharger.')}applyExcelPlan(plan,mode,adapt);localStorage.setItem('lifeos:last-excel-import',JSON.stringify({file:fileName,at:new Date().toISOString(),records:plan.totalRecords,sheets:plan.workbook.sheets.length,formulas:plan.workbook.formulaCount,detected:plan.detected,deepAnalysis:plan.deepAnalysis}));setApplied(true);showToast(`${plan.totalRecords} donnée(s) importée(s) — lecture profonde terminée`)}
  const downloadCompleteArchive=()=>{if(plan)downloadFile(`archive-${fileName.replace(/\.[^.]+$/,'')}.json`,JSON.stringify(plan.workbook,null,2),'application/json;charset=utf-8')}
  ...
  return <div className="tool-workspace excel-importer">
    <div className="excel-intro"><span><FileSpreadsheet size={26}/></span><div><b>Votre classeur devient votre LifeOS</b><p>La lecture profonde parcourt chaque feuille et chaque cellule non vide, déduit le bon module grâce aux noms, dates, heures, montants, statuts, liens et textes, puis archive intégralement toute mise en page atypique dans Notes.</p></div><Badge tone="success"><ShieldCheck size={12}/>100 % local</Badge></div>
    {!plan&&!busy&&<><div className="deep-method"><Sparkles size={19}/><span><b>Lecture profonde activée automatiquement</b><small>Colonnes non standard, tableaux multiples, plannings visuels et feuilles narratives sont compris sans imposer le modèle LifeOS. Rien n’est envoyé sur Internet.</small></span></div><div className="drop-zone excel-drop" role="button" tabIndex={0} onClick={()=>inputRef.current?.click()} onKeyDown={(event)=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();inputRef.current?.click()}}} onDragOver={(event)=>event.preventDefault()} onDrop={(event)=>{event.preventDefault();analyze(event.dataTransfer.files[0])}}><Upload size={32}/><b>Déposez votre fichier Excel ici</b><span>.xlsx, .xlsm, .xltx ou .csv · 25 Mo maximum</span><Button type="button" size="sm">Choisir un fichier</Button></div><input ref={inputRef} type="file" accept=".xlsx,.xlsm,.xltx,.csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/csv" hidden onChange={(event)=>analyze(event.target.files?.[0])}/>{archiveInfo&&<div className="excel-archive-callout"><FileCheck2 size={20}/><span><b>Dernier classeur conservé localement : {archiveInfo.fileName}</b><small>{archiveInfo.sheets} feuilles · {archiveInfo.cells.toLocaleString('fr-FR')} cellules · {date(archiveInfo.savedAt,{day:'2-digit',month:'short',year:'numeric'})}</small></span><Button variant="secondary" size="sm" onClick={reopenArchive}>Relire l’analyse</Button></div>}<div className="excel-template"><TableProperties size={20}/><span><b>Vous partez de zéro ?</b><small>Le modèle contient 11 feuilles déjà structurées pour LifeOS.</small></span><Button variant="secondary" size="sm" onClick={downloadExcelTemplate}><Download size={15}/>Télécharger le modèle Excel</Button></div></>}
    ...
    {plan&&<>
      ...
      {plan.detected.some((item)=>!item.deep)&&<section className="excel-section"><header><span><Check size={17}/><b>Structures LifeOS reconnues</b></span><small>{plan.detected.filter((item)=>!item.deep).length} tableau(x) standard</small></header><div className="detected-grid">{plan.detected.filter((item)=>!item.deep).map((item,index)=><div key={`${item.sheet}-${item.kind}-${index}`}><Check size={15}/><span><b>{item.label}</b><small>Feuille « {item.sheet} » · en-têtes ligne {item.headerRow}</small></span><Badge tone={item.count?'success':'neutral'}>{item.count}</Badge></div>)}</div></section>}
      {plan.deepAnalysis.length>0&&<section className="excel-section deep-analysis"><header><span><Sparkles size={17}/><b>Lecture profonde des feuilles atypiques</b></span><Badge tone="success">{plan.deepAnalysis.length} feuille(s) associée(s)</Badge></header><div className="deep-summary"><span><b>{plan.deepAnalysis.reduce((sum,item)=>sum+item.cells,0).toLocaleString('fr-FR')}</b><small>cellules lues</small></span><span><b>{plan.deepAnalysis.reduce((sum,item)=>sum+item.inferredRecords,0)}</b><small>éléments déduits</small></span><span><b>{plan.deepAnalysis.reduce((sum,item)=>sum+item.archiveNotes,0)}</b><small>archives complètes</small></span></div><p className="deep-explanation"><ShieldCheck size={15}/>Chaque feuille ci-dessous est maintenant reliée à un module. Sa transcription cellule par cellule reste aussi disponible dans Notes : aucun contenu non standard n’est écarté.</p><details><summary>Voir les associations feuille par feuille</summary><div className="deep-sheet-grid">{plan.deepAnalysis.map((item)=><div key={item.sheet}><span><b>{item.sheet}</b><small>{item.rows} lignes · {item.cells} cellules</small></span><strong>{item.destination}</strong></div>)}</div></details></section>}
      ...
      {Object.keys(plan.settings).length>0&&<section className="excel-section imported-formats"><header><span><Palette size={17}/><b>Formats d’affichage détectés</b></span><small>Appliqués dans tout LifeOS après confirmation</small></header><div>{Object.entries(plan.settings).filter(([key])=>importedSettingLabels[key]).map(([key,value])=><span key={key}><small>{importedSettingLabels[key]}</small><b>{String(value)}</b></span>)}</div></section>}
      ...
      <div className="excel-actions">{applied?<><div className="excel-success"><Check size={18}/><span><b>Lecture profonde et personnalisation terminées</b><small>Les données et le classeur complet sont conservés localement.</small></span></div><Button variant="secondary" onClick={downloadCompleteArchive}><Download size={15}/>Archive complète JSON</Button><Button onClick={()=>setView('dashboard')}>Voir mon dashboard →</Button></>:<><Button variant="ghost" onClick={downloadCompleteArchive}><Download size={15}/>Archive complète JSON</Button><Button variant="secondary" onClick={downloadExcelTemplate}><TableProperties size={15}/>Modèle Excel</Button><Button disabled={!plan.totalRecords} onClick={()=>void apply()}><Sparkles size={16}/>Personnaliser LifeOS avec {plan.totalRecords} donnée(s)</Button></>}</div>
    </>}
  </div>
}
