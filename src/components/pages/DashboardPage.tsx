...
export function DashboardPage() {
  ...
  const orderedIds = useMemo(() => [...new Set<ModuleId>([
    ...layout.map((item) => item.i as ModuleId),
  ...
  return <motion.div className="page dashboard-page" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <div className="dashboard-toolbar"><div><h2>Votre espace</h2><p>{activeCount} modules actifs · Chaque carte peut être déplacée et redimensionnée</p></div><Button variant={editMode ? 'primary' : 'secondary'} onClick={() => setEditMode(!editMode)}>{editMode ? <X size={16}/> : <Pencil size={16}/>} {editMode ? 'Terminer' : 'Personnaliser'}</Button></div>

    {editMode && <motion.section className="customization-studio" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}>
      <header><span><Move size={18}/><span><b>Mode personnalisation actif</b><small>Glissez n’importe quelle carte, redimensionnez-la ou choisissez une suggestion.</small></span></span><button onClick={() => setEditMode(false)} aria-label="Fermer la personnalisation"><X size={17}/></button></header>
      <div className="customization-studio-body">
        <div className="preset-area"><b><LayoutTemplate size={15}/>Suggestions de disposition</b><div className="dashboard-presets">{dashboardPresets.map((preset) => {
          const selected = activeCount === preset.modules.length && preset.modules.every((id) => visible[id] !== false)
          return <button key={preset.id} className={selected ? 'active' : ''} onClick={() => { applyPreset(preset.id); showToast(`Disposition « ${preset.label} » appliquée`) }}><span>{selected ? <Check size={15}/> : <LayoutTemplate size={15}/>}</span><b>{preset.label}</b><small>{preset.description}</small></button>
        })}</div></div>
    ...
    {!mobile && <div className={`desktop-grid ${editMode ? 'editing' : ''}`}><ResponsiveGrid className="layout" layouts={{ lg: layout as Layout[] }} breakpoints={{ lg: 1100, md: 900, sm: 640, xs: 420, xxs: 0 }} cols={{ lg: 12, md: 8, sm: 4, xs: 2, xxs: 1 }} rowHeight={54} margin={[16, 16]} compactType="vertical" isDraggable={editMode} isResizable={editMode} draggableCancel="button, input, textarea, select, a, [contenteditable='true']" resizeHandles={['se']} onLayoutChange={(_, all) => { if (all.lg) setLayout(all.lg) }} useCSSTransforms>{children}</ResponsiveGrid></div>}
