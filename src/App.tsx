...
export default function App(){
  ...
  useEffect(()=>{const hash=()=>{const raw=location.hash.replace('#/','').split('/')[0];const next=valid.includes(raw as View)?raw as View:'dashboard';if(next!==useUIStore.getState().view)useUIStore.setState({view:next})};addEventListener('hashchange',hash);return()=>removeEventListener('hashchange',hash)},[])
