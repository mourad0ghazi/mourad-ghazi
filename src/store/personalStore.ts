...
interface PersonalState {
  ...
  noteReadingProgress: Record<string, { scrollTop: number; cursor: number }>
  ...
  setNoteReadingProgress: (id: string, progress: { scrollTop: number; cursor: number }) => void
...
export const usePersonalStore = create<PersonalState>()(persist((set) => ({
  tasks: initialTasks, notes: initialNotes, habits: initialHabits, journal: initialJournal, goals: initialGoals, events: initialEvents, noteReadingProgress: {},
  ...
  removeNote: (id) => set((state) => { const noteReadingProgress = { ...state.noteReadingProgress }; delete noteReadingProgress[id]; return { notes: state.notes.filter((note) => note.id !== id), noteReadingProgress } }),
  setNoteReadingProgress: (id, progress) => set((state) => ({ noteReadingProgress: { ...state.noteReadingProgress, [id]: progress } })),
  ...
  reset: () => set({ tasks: initialTasks, notes: initialNotes, habits: initialHabits, journal: initialJournal, goals: initialGoals, events: initialEvents, noteReadingProgress: {} }),
